...
// Aggregate platform stats (transaction count + total USDC volume)
app.get('/stats', async (c: Context) => {
  try {
    // Total transaction count
    const [{ count }] = await db.select({ count: sql`count(*)` }).from(rpcLogs);

    // Compute total USDC volume from payment metadata
    // Payment value lives in meta -> payment -> paymentRequest (base64-encoded JSON) -> payload -> authorization -> value
    // Or in response _meta -> x402/payment-response
    const rows = await db
      .select({ meta: rpcLogs.meta, response: rpcLogs.response, request: rpcLogs.request })
      .from(rpcLogs);

    let totalVolumeMicro = 0;
    for (const row of rows) {
      try {
        // Try to extract value from request params _meta x402/payment (base64 JSON)
        const req = row.request as any;
        const res = row.response as any;
        const meta = row.meta as any;

        // Check meta.payment for paymentRequest
        const paymentToken = meta?.['x402/payment'] || req?.params?._meta?.['x402/payment'];
        if (paymentToken && typeof paymentToken === 'string') {
          try {
            const decoded = Buffer.from(paymentToken, 'base64').toString('utf8');
            const parsed = JSON.parse(decoded);
            const val = parsed?.payload?.authorization?.value;
            if (val && !isNaN(Number(val))) {
              totalVolumeMicro += Number(val);
            }
          } catch {}
        }

        // Also check response for payment-response with value
        const payRes = res?._meta?.['x402/payment-response'];
        if (payRes && !paymentToken) {
          // If we didn't get it from request, try response
          const val = payRes?.value;
          if (val && !isNaN(Number(val))) {
            totalVolumeMicro += Number(val);
          }
        }
    ...
    const totalVolumeUsd = totalVolumeMicro / 1e6;
    ...
    return c.json({ totalTransactions: 0, totalVolumeUsd: 0 });
  }
});
