...
async function getToolsFromMCP(url: string, authHeaders?: Record<string, string>) {
  try {
    ...
    const client = new Client({ name: "clawpay-inspect-tools", version: "1.0.0" })
    await withTimeout(client.connect(transport), 8000, undefined as any)
    const result = await withTimeout(client.listTools(), 5000, { tools: [] })
    // Close transport to avoid hanging connections
...
async function getPromptsFromMCP(url: string, authHeaders?: Record<string, string>) {
  try {
    ...
    const client = new Client({ name: "clawpay-inspect", version: "1.0.0" })
    await withTimeout(client.connect(transport), 8000, undefined as any)
    const prompts = await withTimeout(client.listPrompts(), 5000, { prompts: [] })
