...
const defaultTemplates: Templates = {
  json: ({ baseUrl, apiKey, serverId }, authMode = 'api_key', platform = 'mac') => {
    ...
    const getArgs = () => {
      if (platform === 'win') {
        return ['/c', '@clawpay-hedera/sdk', 'connect', '--urls', baseUrl]
      }
      if (platform === 'wsl') {
        return ['@clawpay-hedera/sdk', 'connect', '--urls', baseUrl]
      }
      return ['@clawpay-hedera/sdk', 'connect', '--urls', baseUrl]
    }
...
const generateRaycastDeepLink = (server: ServerInfo, apiKey?: string) => {
  const config = {
    ...
    type: "stdio",
    command: "npx",
    args: ["@clawpay-hedera/sdk", "connect", "--urls", server.baseUrl]
  }
                ...
                <div className="space-y-3">
                  <div className="text-xs text-muted-foreground">
                    {state.codeLanguage === 'ts' 
                      ? 'npm install @modelcontextprotocol/sdk clawpay x402'
                      : 'pip install mcp'
                    }
                  </div>
