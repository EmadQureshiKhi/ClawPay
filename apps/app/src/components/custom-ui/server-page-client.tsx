...
export function ServerPageClient({ serverId, initialData }: ServerPageClientProps) {
  ...
  // Derive HCS audit topic from tool annotations (set by withX402 plugin)
  const hcsTopic = useMemo(() => {
    if (!data?.tools) return undefined
    for (const t of data.tools) {
      const annotations = (t as { annotations?: Record<string, unknown> })?.annotations || {}
      const topic = annotations.hcsTopic as string | undefined
  ...
  const mcpUrlDisplay = useMemo(() => {
    ...
    try {
      // Use the mcp2 format: https://mcp2.clawpay.tech/mcp?id=serverId
