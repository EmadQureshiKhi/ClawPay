...
const LOG_PREFIX = "[ClawPay][ToolExecutionModal]"
const log = {
...
export function ToolExecutionModal({ isOpen, onClose, tool, serverId, url }: ToolExecutionModalProps) {
  ...
  // Hedera wallets with stored encrypted keys are signed server-side by the proxy — no browser wallet needed
  const isHederaWallet = activeWallet?.blockchain === 'hedera-testnet' || activeWallet?.blockchain === 'hedera'
  const hasStoredKey = isHederaWallet && !!(activeWallet?.walletMetadata as Record<string, unknown> | undefined)?.encryptedKey
  const requiresBrowserConnection = activeWallet?.walletType === 'external' && !isHederaWallet
  ...
  const needsWalletClient = requiresBrowserConnection // Only non-Hedera external wallets need wagmi wallet client

  // Overall connection status — Hedera wallets with stored keys are always "connected"
  const isConnected = hasAccountWallets && (isHederaWallet || ((!needsWalletClient || hasWalletClient) && !needsBrowserConnection))
  ...
  useEffect(() => {
    ...
    const initializeMcpClient = async () => {
      try {
        ...
        const mcpClient = new Client({ name: "clawpay-client", version: "1.0.0" })
        const networkForSigner = getRequiredNetwork() || getCurrentNetwork() || 'base'
        let evmSigner: MultiNetworkSigner['evm'] | undefined

        if (isHederaWallet) {
          // Hedera wallets: proxy signs server-side using stored encrypted key — no client-side signer needed
          log.info("Using Hedera wallet - client-side signing disabled; proxy will sign via stored key", {
            address: walletAddress,
            hederaAccountId: (activeWallet?.walletMetadata as Record<string, unknown> | undefined)?.hederaAccountId,
          })
        } else if (activeWallet?.walletType === 'external') {
          ...
          // Managed wallets: server will auto-sign via stored key
        ...
        for (const t of toolsResult.tools) {
          ...
          toolsCollection[toolName] = {
            ...
            execute: async (params: Record<string, unknown>, options: { toolCallId: string; messages: unknown[] }) => {
              ...
              // Extract HCS audit metadata from _meta if present
              const hcsMeta = (res._meta && isRecord(res._meta)) ? {
                hcsTopic: res._meta['clawpay/hcs-topic'] as string | undefined,
                hcsUrl: res._meta['clawpay/hcs-url'] as string | undefined,
              } : {}
              ...
              if (textItems.length > 0) {
                ...
                try {
                  ...
                  // Attach HCS metadata to parsed result
                  if (hcsMeta.hcsTopic && typeof parsed === 'object' && parsed !== null) {
                    parsed.__hcsTopic = hcsMeta.hcsTopic
                    parsed.__hcsUrl = hcsMeta.hcsUrl
                  }
                  return parsed
                } catch {
                  ...
                  // For text results with HCS metadata, wrap in object
                  if (hcsMeta.hcsTopic) {
                    return { __text: combinedText, __hcsTopic: hcsMeta.hcsTopic, __hcsUrl: hcsMeta.hcsUrl }
                  }
                  return combinedText
                }
              }
  ...
  const executeTool = async () => {
    ...
    try {
      ...
      if (typeof result === 'object' && result !== null) {
        ...
        hcsTopic = resultObj.__hcsTopic as string | undefined
        hcsUrl = resultObj.__hcsUrl as string | undefined
      }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-foreground">
                {isHederaWallet
                  ? `Hedera ${(activeWallet?.walletMetadata as Record<string, unknown> | undefined)?.hederaAccountId || formatWalletAddress(walletAddress || '')}`
                  : activeWallet?.walletType === 'managed'
                    ? 'ClawPay Wallet'
                    : (activeWallet?.provider || 'Wallet')}{!isHederaWallet ? ` (${formatWalletAddress(walletAddress || '')})` : ''}
              </span>
              {isHederaWallet && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-sm bg-teal-500/10 text-teal-600 dark:bg-teal-800/50 dark:text-teal-400">
                  {activeWallet?.blockchain === 'hedera-testnet' ? 'Testnet' : 'Mainnet'} · Proxy-signed
                </span>
              )}
  ...
  const renderExecutionResult = () => {
    if (execution.status === 'success' && execution.result) {
      const formatResult = () => {
        ...
        // Helper to strip internal HCS metadata fields from display
        const stripInternalFields = (obj: unknown): unknown => {
          if (typeof obj !== 'object' || obj === null) return obj
          const cleaned = { ...obj as Record<string, unknown> }
          delete cleaned.__hcsTopic
          delete cleaned.__hcsUrl
          // If it was a wrapped text result, unwrap it
          if ('__text' in cleaned && Object.keys(cleaned).length <= 1) {
            return cleaned.__text
          }
          delete cleaned.__text
          return cleaned
        }
        ...
        try {
          const cleaned = stripInternalFields(execution.result)
          if (typeof cleaned === 'string') return cleaned
          return showPrettyJson ? JSON.stringify(cleaned, null, 2) : JSON.stringify(cleaned)
        } catch {
