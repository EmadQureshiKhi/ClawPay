...
export function AccountModal({ isOpen, onClose, defaultTab }: AccountModalProps) {
    ...
    <div className="flex flex-col h-full p-6 sm:p-8">
      ...
      <div className="text-center space-y-6">
        ...
        <div className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-2xl mx-auto flex items-center justify-center transition-all duration-300 ${isDark ? "bg-gradient-to-br from-gray-800/80 to-gray-900/80 shadow-lg" : "bg-gradient-to-br from-gray-100 to-gray-50 shadow-md"}`}>
          <img src="/clawpay-logo.png" alt="ClawPay" width={48} height={48} className="rounded-lg" />
          <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r from-teal-500/10 to-emerald-500/10 animate-pulse`} />
        </div>
        
        {/* Title and Description */}
        <div className="space-y-3">
          <h2 className={`text-2xl sm:text-3xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}>Welcome to <span>Claw</span><span className="text-neutral-400">Pay</span></h2>
  ...
  if (sessionLoading || isAuthenticating) {
    ...
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className={`max-w-xl h-[72vh] flex flex-col p-0 ${isDark ? "!bg-black border-neutral-800" : "!bg-white border-neutral-200"}`}>
  ...
  if (isMobile) {
    return (
      <Drawer open={isOpen} onOpenChange={onClose}>
        <DrawerContent className={`h-[72vh] flex flex-col p-0 ${isDark ? "!bg-black border-neutral-800" : "!bg-white border-neutral-200"}`}>
