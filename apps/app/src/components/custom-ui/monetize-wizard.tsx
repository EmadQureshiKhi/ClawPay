...
export function MonetizeWizard({ open, onOpenChange, serverUrl, tools, initialAuthHeaders, initialRequireAuth, onCreate }: MonetizeWizardProps) {
  ...
  // Hedera-only: override network lists
  const hederaEvmNetworks = ['hedera-testnet', 'hedera'] as const
  const hederaSvmNetworks = [] as const
  const evmNetworks = hederaEvmNetworks as unknown as typeof SupportedEVMNetworks
  const svmNetworks = hederaSvmNetworks as unknown as typeof SupportedSVMNetworks
  const visibleEvmNetworks = recipientIsTestnet ? hederaEvmNetworks.filter(n => n === 'hedera-testnet') : hederaEvmNetworks.filter(n => n === 'hedera')
  const visibleSvmNetworks: string[] = []
  ...
  const stepDescription = useMemo(() => {
    switch (step) {
      ...
      case 4: return 'Enter Hedera EVM recipient addresses. You can edit these later.'
      default: return ''
    }
  }, [step])
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              <div className="bg-card rounded-[2px] p-3">
                <div className="text-lg font-mono font-medium text-foreground mb-2">HEDERA</div>
            ...
            {needsEvm && (
              <div className="space-y-2">
                <div className="grid grid-cols-[1fr_auto_auto_auto] gap-2 items-end">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <Label className="text-sm font-mono uppercase">HEDERA EVM RECIPIENT</Label>
                  ...
                  <Popover open={evmWalletSelectorOpen} onOpenChange={setEvmWalletSelectorOpen}>
                    {evmWallets.length === 0 ? (
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>Connect to ClawPay to select a wallet from here</p>
                        </TooltipContent>
                      </Tooltip>
                    ) : (
                      <PopoverTrigger asChild>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl md:max-w-3xl lg:max-w-4xl w-[min(96vw,900px)] min-h-[80vh] max-h-[80vh] flex flex-col rounded-[2px]">
        <DialogHeader className="pb-6">
          <DialogTitle className="sr-only">Monetize Server</DialogTitle>
