...
export const urlUtils = {
  ...
  getApi2Url: () => (env as Record<string, unknown>).NEXT_PUBLIC_API2_URL as string ?? 'https://api2.clawpay.tech',
...
export const authApi = {
  ...
  // Link Hedera wallet (MetaMask address + encrypted private key)
  linkHederaWallet: async (data: {
    walletAddress: string;
    hederaAccountId: string;
    privateKey: string;
    network?: string;
  }) => {
    return serviceApiCall(urlUtils.getAuthUrl(), '/api/wallets/hedera', {
      method: 'POST',
  ...
  getHederaBalance: async (accountId: string, network: string = 'testnet') => {
    return serviceApiCall(urlUtils.getAuthUrl(), `/api/wallets/hedera/balance?accountId=${encodeURIComponent(accountId)}&network=${encodeURIComponent(network)}`)
  },
}
...
export const mcpDataApi = {
  ...
  getStats: async (): Promise<{
    ...
    return serviceApiCall(urlUtils.getMcpDataUrl(), `/stats`)
  },
