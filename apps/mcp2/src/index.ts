...
import { AuthHeadersHook, LoggingHook, withProxy, X402MonetizationHook } from "@clawpay-hedera/sdk/handler";
...
async function buildMonetizationForTarget(targetUrl: string): Promise<{
    ...
    try {
        ...
        const hederaNetworks = ['hedera-testnet', 'hedera'];
        ...
        if (server.recipient?.evm?.address) {
            if (selectedNetworks && selectedNetworks.length > 0) {
                const selectedHederaNetworks = selectedNetworks.filter(n => hederaNetworks.includes(n));
                for (const network of selectedHederaNetworks) {
                    recipient[network as Network] = server.recipient.evm.address;
                }
            } else {
                // Map based on testnet flag — Hedera only
                const isTestnet = server.recipient.evm.isTestnet;
                const targetNetworks = isTestnet ? ['hedera-testnet'] : ['hedera'];
        ...
        // SVM not supported on Hedera — skip
