...
export class X402MonetizationHook implements Hook {
    ...
    async processCallToolResult(res: CallToolResult, original: CallToolRequest, extra: RequestExtra) {
        ...
        try {
            ...
            const selected = findMatchingPaymentRequirements(accepts, decoded);
            if (!selected) return { resultType: "continue" as const, response: res };
            ...
            if (s.success) {
                ...
                const content = [];
                if (Array.isArray(res.content)) content.push(...res.content);
                content.push({ type: "text", text: `Payment settled on ${s.network} (tx: ${s.transaction ?? "n/a"}).` } as TextContent);
                return { resultType: "continue" as const, response: { ...res, _meta: meta, content } };
            }
            return { resultType: "continue" as const, response: this.paymentRequired([], s.errorReason ?? "SETTLEMENT_FAILED") };
        } catch {
            return { resultType: "continue" as const, response: this.paymentRequired([], "SETTLEMENT_FAILED") };
        }
    }
